...
grep -Fq 'const REPO_PATH = resolve(process.env.HOME!, "projects/pi-config");' "$source_file"
...
if grep -Eq '~/(dev|projects-old)/pi-config|/home/ops/dev/pi-config' \
  "$source_file" "$root/AGENTS.md" "$root/README.md"; then
  echo 'a legacy pi-config path is still present' >&2
  exit 1
fi

echo 'ok - pi-sync is fixed to $HOME/projects/pi-config'
